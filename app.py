...
load_dotenv()
# Now access the variables like this:
...
class resume_analyzer:
    ...
    def groq_llm(groq_api_key, chunks, analyze):
        # Step 1: Use HuggingFace embeddings instead of OpenAI embeddings
    ...
    def resume_summary():
        ...
        if submit:
            if pdf is not None and groq_api_key != '':
                try:
                    with st.spinner('Processing...'):
                        ...
                        summary = resume_analyzer.groq_llm(groq_api_key=groq_api_key, chunks=pdf_chunks, analyze=summary_prompt)
                ...
                st.markdown(f'<h5 style="text-align: center;color: orange;">Please Enter GROQ API Key</h5>', unsafe_allow_html=True)
        
    def analysis_prompt(query_with_chunks):
        query = f'''Provide a comprehensive analysis of the below resume including:

1. STRENGTHS: Identify and explain the strong points, achievements, and positive aspects
2. WEAKNESSES: Identify areas for improvement and potential gaps
3. SUGGESTIONS: Provide specific, actionable recommendations to enhance the resume
4. OVERALL ASSESSMENT: Give a balanced evaluation with a final conclusion
            ...
            if pdf is not None and groq_api_key != '':
                try:
                    with st.spinner('Analyzing your resume...'):
                        ...
                        analysis_prompt = resume_analyzer.analysis_prompt(query_with_chunks=summary)

                        analysis = resume_analyzer.groq_llm(groq_api_key=groq_api_key, chunks=pdf_chunks, analyze=analysis_prompt)

                    st.markdown(f'<h4 style="color: orange;">📊 Comprehensive Resume Analysis:</h4>', unsafe_allow_html=True)
                    st.write(analysis)
            ...
            if pdf is not None and groq_api_key != '':
                try:
                    with st.spinner('Processing...'):
                        ...
                        job_title = resume_analyzer.groq_llm(groq_api_key=groq_api_key, chunks=pdf_chunks, analyze=job_title_prompt)
...
with st.sidebar:
    ...
    option = option_menu(menu_title='', options=['Welcome', 'Summary', 'Analysis', 'Job Titles', 'Linkedin Jobs'],
                         icons=['house-fill', 'house-fill', 'clipboard-data', 'list-ul', 'linkedin'])

if option == 'Welcome':
    
    st.markdown("""
    # 🎯 Welcome to AI Resume Analyzer & LinkedIn Scraper
    
    ## 🚀 What You Can Do:
    
    ### 📄 Resume Analysis
    - **Summary**: Get a quick overview of your resume
    - **Analysis**: Comprehensive strengths, weaknesses, and improvement suggestions
    - **Job Titles**: Get personalized job recommendations
    
    ### 🔍 LinkedIn Scraper
    - Search for jobs by title and location
    - Get company details and job descriptions
    - Find direct application links
    
    ## 🔑 What You Need:
    1. **Groq API Key** (free from https://console.groq.com/)
    2. **PDF Resume** for analysis
    3. **Job title** for LinkedIn search
    
    ## 📋 Quick Start:
    1. Get your Groq API key from https://console.groq.com/
    2. Choose an analysis option from the sidebar
    3. Upload your resume and enter your API key
    4. Get instant AI-powered insights!
    
    ---
    *Ready to analyze your resume? Choose an option from the sidebar!*
    """)
